...
import glob, os, shutil, warnings, sys
...
def main():
    ...
    parser.add_argument('--version', action='version', version=f'mc-alf {__version__}')
    parser.add_argument('config')
    args = parser.parse_args()

    configpars = fitter.readconfig(args.config)

    print(f'MC-ALF version {__version__}')

    if args.debug:
       print('--- DEBUG mode, increased verbosity ---')
    if args.debug:
        print('[DEBUG]: hires_fitter loaded from {}'.format(fitter.__file__))
    ...
    if configpars['dofit']:
            ...
            with fitter.als_fitter(configpars['specfile'], configpars['wavefit'], configpars['linelist'], configpars['ncomp'], \
               ...
               if configpars['solver'] == 'polychord':
                  ...
                  settings.feedback = 0
                  if 'pc_settings' in configpars:
                     configsettings = configpars['pc_settings']
               ...
               elif configpars['solver'] == 'dypolychord': #Never deeply tested
               ...
               elif configpars['solver'] == 'multinest': #Too slow with tens of parameters
                  ...
                  if mpi==1:
                   ...
                   try:
                       ...
                       import jaxns.internals.mixed_precision as jmp
                       jmp.mp_policy = jmp.Policy(count_dtype=jnp.int32, index_dtype=jnp.int32, measure_dtype=jnp.float32)
                   ...
                   lowers_j = jnp.array(lowers, dtype=jnp.float32)
                   uppers_j = jnp.array(uppers, dtype=jnp.float32)
                   
                   def prior_model():
                       p = yield Prior(tfp.distributions.Uniform(low=lowers_j, high=uppers_j), name='p')
                       return p
                   
                   # JAXNS Model Force the Model creation to happen on CPU memory
                   ...
                   #Actual run here
                   t0 = datetime.datetime.now()
                   # Use seed 43 to refresh
                   termination_reason, state = ns(key=jax.random.PRNGKey(jnp.int32(43)))
                   t1 = datetime.datetime.now()
                   print('[INFO] JAXNS Execution time {}'.format(t1-t0))
                   ...
                   key = jax.random.PRNGKey(jnp.int32(42))
                   ...
                   print(f"Saved JAXNS results to {chain_file}")

               else:
                   print('Requested solver {} not implemented'.format(configpars['solver']))
    ...
    if configpars['doplot']:
         ...
         if os.path.exists(configpars['chaindir']+configpars['chainfmt'].format(configpars['nfill'])+'_equal_weights.txt'): 
                ...
                with fitter.als_fitter(configpars['specfile'], configpars['wavefit'], configpars['linelist'], configpars['ncomp'], \
                  ...
                  #specresolution needs to be a scalar not an array, even if 1 dimensional
                  if len(temp.specres)>1:
                     specresolution = meds[0]
                  else:
                     specresolution = temp.specres[0]
                  
                  ncomp_arr, ncomp_counts = np.unique(np.floor(values[:,temp.startind]).astype(int), return_counts=True)
